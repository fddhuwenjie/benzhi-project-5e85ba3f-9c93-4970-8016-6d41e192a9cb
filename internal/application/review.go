...
func (s *Service) Authorize(ctx context.Context, id string, input AuthorizationInput) (*Result, error) {
	...
	page, err := s.store.Audit(ctx, id, "", 10000, 0)
	if err != nil {
		return nil, err
	}
	...
	err = s.store.Commit(ctx, repository.Commit{Release: release, ExpectedRevision: input.ExpectedRevision, Event: event, Idempotency: record, Evidence: evidence, EvidenceDigest: digest})
	if err != nil {
		return nil, err
	}
	return result, nil
