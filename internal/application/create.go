...
func (s *Service) Create(ctx context.Context, input CreateInput) (*Result, error) {
	...
	if result, ok, err := s.replay(ctx, input.RequestID, operation, fp); err != nil || ok {
		return result, err
	}
	check, err := s.Precheck(ctx, PrecheckInput{Title: input.Title, Objective: input.Objective, ModelCode: input.ModelCode, PlannedCondition: input.PlannedCondition, Owner: input.Owner})
	if err != nil {
		return nil, err
	}
	...
	err = s.store.Create(ctx, repository.Commit{Release: release, ExpectedRevision: 0, Event: event, Idempotency: record})
	if err != nil {
		if replayed, ok, replayErr := s.replay(ctx, input.RequestID, operation, fp); replayErr == nil && ok {
			return replayed, nil
		}
