...
type Service struct {
	...
	evidenceLoaders map[string]*evidenceLoader
}
...
func newEvidenceLoader(store repository.Store, ctx context.Context, releaseID string) *evidenceLoader {
	return &evidenceLoader{store: store, ctx: ctx, releaseID: releaseID}
}

func (l *evidenceLoader) Load() ([]byte, string, error) {
	return l.store.Evidence(l.ctx, l.releaseID)
}
...
func New(store repository.Store, auditService *audit.Service, now func() time.Time) *Service {
	...
	return &Service{store: store, audit: auditService, now: now, evidenceLoaders: make(map[string]*evidenceLoader)}
}
