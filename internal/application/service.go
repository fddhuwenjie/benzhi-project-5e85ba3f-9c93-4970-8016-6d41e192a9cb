...
func (s *Service) mutate(ctx context.Context, releaseID, operation string, meta CommandMeta, requiredRole domain.Role, input any, fn mutation) (*Result, error) {
	...
	if result, ok, err := s.replay(context.WithoutCancel(ctx), meta.RequestID, operation, fp); err != nil || ok {
		return result, err
	}
	release, err := s.store.Load(context.WithoutCancel(ctx), releaseID)
	if err != nil {
		return nil, err
	}
